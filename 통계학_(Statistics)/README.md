<div align="center">
  <a href="https://sugar-spider-af5.notion.site/34becff3027980f4bc75cf1c7600e033?source=copy_link">
    <img src="https://img.shields.io/badge/Notion-Study--Log-000000?style=for-the-badge&logo=notion&logoColor=white" />
  </a>

  # 📊 Statistics & Data Analysis Technical Log
  **로봇 센서 노이즈 극복과 신뢰성 있는 데이터 분석을 위한 통계학 로그**
</div>

## 📌 Focus
- **노이즈/이상치 제어**: 로봇 센서(LiDAR, IMU, 등)에서 발생하는 이상치들을 통계적으로 판별하고 제거하여 데이터의 신뢰성을 확보
- **데이터 정규화**: 서로 다른 단위를 가진 센서 데이터를 정규화하여 로봇 제어 알고리즘이나 학습 모델의 연산 효율을 최적화

---

### 🔗 Quick Links
- **통계학 정리본 (Notion)** &nbsp; ➔ &nbsp; **"** [![Notion](https://img.shields.io/badge/Open_Notion-000000?style=flat&logo=notion&logoColor=white)](https://sugar-spider-af5.notion.site/34becff3027980f4bc75cf1c7600e033?source=copy_link) **"**

> [!NOTE]
> 디자인 레이아웃 유지를 위해 노션 원본 링크로 공유하며, 무단 복제는 허용하지 않습니다.

---

<p>
  © 2026 Seong-hun Bae. &nbsp; All rights reserved.
</p>
